...
Bitcoin.prototype.getAPIMethods = function () {
    var methods = [
        ...
        ['sendToAddress', this, this.sendToAddress, 1],
        ['getAccountAddress', this, this.getAccountAddress, 1],
        ['spork', this, this.spork, 1]
    ];
    return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function (node) {
    ...
    var configPath = path.resolve(spawnOptions.datadir, './firo.conf');
...
Bitcoin.prototype._getNetworkConfigPath = function () {
    var networkPath;
    if (this.node.network === bitcore.Networks.testnet) {
        networkPath = 'testnet3/firo.conf';
        if (this.node.network.regtestEnabled) {
            networkPath = 'regtest/firo.conf';
        }
    }
    return networkPath;
};
...
Bitcoin.prototype._getNetworkOption = function () {
    ...
    } else if (this.node.network === bitcore.Networks.devnet) {
...
Bitcoin.prototype._getAddressUnconfirmedBalance = function (deltas, address, callback) {
  var txidMap = new Map();
  var self = this;
  ...
  async.map(txidVec, function (txid, next) {
    ...
    for(var i=0; i < detailed.length; ++i) {
      if(detailed[i] != null
        ...
        && detailed[i].hasOwnProperty('height')) {
        if(detailed[i].height == -1) {
          detailed[i].inputs.forEach(function(input){
              if(input.address == address){
                bal -= input.satoshis;
                txno += 1;
              }
          });
          detailed[i].outputs.forEach(function(output){
    ...
    mempoolTxids = {txNumber: txno , balance: bal };
    callback(null, mempoolTxids);
  });
};
        ...
        async.parallel([
            ...
            function getMempool(done) {
                ...
                self.client.getAddressMempool({'addresses': addresses}, function (err, response) {
                    ...
                    self._getAddressUnconfirmedBalance(response.result, addresses[0], function(err, result){
                      summary.unconfirmedAppearances = result.txNumber;
                      summary.unconfirmedBalance = result.balance;
                      done();
                    });
                });
            },
        ...
        self._tryAllClients(function (client, done) {
            client.getRawTransaction(txid, 1, function (err, response) {
                ...
                if (tx.coinbase) {
                  ...
                  if (result.vin[0] && result.vin[0].hasOwnProperty('nFees')) {
                    tx.feeSatoshis = result.vin[0].nFees * 1e+8;
                  } else {
                    tx.feeSatoshis = tx.inputSatoshis - tx.outputSatoshis;
                  }
                }

                tx.extraPayload = tx.version >= 3 && tx.type != 0;

                if (tx.extraPayload) {
                    if (tx.version >= 3 && tx.type == 1) { //TRANSACTION_PROVIDER_REGISTER
                        tx.proReg = result.proReg;
                    }
                    if (tx.version >= 3 && tx.type == 2) { //TRANSACTION_PROVIDER_UPDATE_SERVICE
                        tx.proUpServ = result.proUpServ;
                    }
                    if (tx.version >= 3 && tx.type == 3) { //TRANSACTION_PROVIDER_UPDATE_REGISTRAR
                        tx.proUpReg = result.proUpReg;
                    }
                    if (tx.version >= 3 && tx.type == 4) { //TRANSACTION_PROVIDER_UPDATE_REVOKE
                        tx.proUpRev = result.proUpRev;
                    }
                    if (tx.version >= 3 && tx.type == 5) { //TRANSACTION_COINBASE
                        tx.cbTx = result.cbTx;
                    }
                    if (tx.version >= 3 && tx.type == 6) { //TRANSACTION_QUORUM_COMMITMENT
                        tx.finalCommitment = result.finalCommitment;
                    }
                    if (tx.version >= 3 && tx.type == 7) { //Spork control TXs
                      tx.sporkTx = result.sporkTx;
                    }
                }
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getWalletInfo = function (callback) {
  var self = this;
  this.client.getWalletInfo(function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var info = {
      balance: response.result.balance,
      txcount: response.result.txcount
    };
    callback(null, info);
  });
};
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getAccountAddress = function (address, callback) {
  var self = this;
  this.client.getAccountAddress(address, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.sendToAddress = function (address, amount, callback) {
  var self = this;
  this.client.sendToAddress(address, amount, '', '', function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var res = {
...
Bitcoin.prototype.evoznodelist = function(callback) {
  var self = this;
  this.client.evoznodelist(function(err, response) {
  if (err) {
    return callback(self._wrapRPCError(err));
  }
  var result = response;
...
Bitcoin.prototype.spork = function(param, callback) {
  var self = this;
  this.client.spork(param, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response;
